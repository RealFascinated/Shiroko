...
**`/permissions clear <role>`**: delete the role's row entirely. The role then resolves to no flags; it should be re-created via `set`. Descendants that pointed at it become dangling-parent, so they inherit no flags.
